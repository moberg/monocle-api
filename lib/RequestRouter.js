...
var debug = require('debug')('monocle-api:request-router');
...
function RequestRouter(request, route, connection) {
    ...
    if (request.isGet()) {
        ...
        for (var key in route.query) {
            ...
            if (typeof request.getQuery(key) !== 'undefined') {
                continue;
            }
...
RequestRouter.ERROR_SCHEMA = Request.ERROR_SCHEMA;
RequestRouter.RESPONSE_ERROR_SCHEMA = Request.RESPONSE_ERROR_SCHEMA;
RequestRouter.ERROR_RESPONSE_INVALID = Request.ERROR_RESPONSE_INVALID;
RequestRouter.ERROR_NO_HANDLER = Request.ERROR_NO_HANDLER;
RequestRouter.ERROR_FORBIDDEN = Request.ERROR_FORBIDDEN;
RequestRouter.ERROR_PROPS_NOT_FOUND = Request.ERROR_PROPS_NOT_FOUND;
RequestRouter.ERROR_ALIAS_DID_NOT_RESOLVE = Request.ERROR_ALIAS_DID_NOT_RESOLVE
RequestRouter.ERROR_ALIAS_ITSELF = Request.ERROR_ALIAS_ITSELF;
...
function castParam(key, value, type) {
    ...
    if (type.indexOf('float') !== - 1 || type.indexOf('number') !== - 1) {
...
function validateAndCastParam(key, value, schemaProperty) {
    ...
    if (value == null) {
        //If value == null but null is not allowed then return error
        if (schemaProperty.type.indexOf('null') !== -1) {
            return null;
        } else {
            return {errorCode: getErrorCode()};
        }
    ...
    for (var key in object) {
        ...
        if (this.isGet()) {
            ...
            if (isArray || isObject) {
                ...
                object[key] = value;
                debug('setting array:', key, value);
            }
        } else {
            var isArray = Array.isArray(value);
            //Check if object is a hash
            var isObject = !isArray && _.isObject(value) && !_.isDate(value) && value !== null;
        }
...
RequestRouter.prototype.setResource = function(resource) {
    ...
    if (this.isPost() && this.isCollection()) {
...
RequestRouter.prototype.error = function(httpStatusCode, errorObject) {
    return this._request.error(httpStatusCode, errorObject, this._propertyErrors);
};
...
RequestRouter.prototype.addPropertyError = function(property, errorCode) {
    //Remove the index of the array if it's added to the end of the property name
    //Ex: paramArray.0 will become paramArray
    var property = property.replace(/(\.[0-9]+)$/, '');
...
//at the same time
RequestRouter.prototype.propertyError = function(property, propertyErrorCode, httpStatusCode, errorObject) {
    this.addPropertyError(property, propertyErrorCode);
    if (!errorObject) {
        errorObject = Request.ERROR_SCHEMA;
    }
    return this.error(httpStatusCode, errorObject);
};
