...
Router.prototype.handle = function (request, connection) {
    ...
    /* TODO: Add support for Monocle middlewares and run the input middlewares here  */
    ...
    /*
        2.
        TODO: monocle-scopes - another middleware that based on the scopes stored in the Monocle request filters the request.

        If the user does not have read access to the resource return 403, otherwise modify the
        request and include only the fields the user has access to read (if a GET request). For any write request the user
        MUST have write access to ALL the fields he is trying to write/update, if not return 403.
     */
...
Router.prototype.middleware = function (options) {
    ...
    if (options.middleware === undefined) {
        options.middleware = function(r, c, next) {
            next();
        }.bind(this);
    }
    ...
    return function (req, res, next) {
        ...
        /*
            TODO: call the middleware for the incoming request. The
        */
