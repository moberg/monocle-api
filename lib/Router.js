...
var SchemaScopes = require('./SchemaScopes');
...
Router.prototype.handle = function (request, connection) {
    //return request.error(403, RequestRouter.ERROR_FORBIDDEN);
    ...
    if (request.isOptions() && -1 !== ['/', ''].indexOf(resourceId)) {
        return Promise.resolve(this.getOpenApiDocumentation());
    }
    ...
    if (request.isOptions()) {
        return Promise.resolve(routeObject.getOpenApiDocumentation());
    }
    ...
    // TODO: this should be provided by the JWT token
    const requestScopes = ['read:scope', 'read:location'];

    const schemaScopes = new SchemaScopes(route.schema);

    // Check if the requester has access to the top level resource.
    // TODO: should only be called on the initial call? Need to handle for Symlinks to sub-resources.
    // if (!schemaScopes.canAccessResource(requestScopes)) {
    //   emit.call(this, 'api:error', requestRouter, null, timeStart);
    //   return requestRouter.error(403, RequestRouter.ERROR_FORBIDDEN);
    // }

    // Filter the incoming requested properties (if any). If no properties filterProps return all that is allowed given
    // the scopes provided
    request.setProps(schemaScopes.filterProps(request.getProps(), requestScopes));
